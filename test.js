...
async function runTests() {
    ...
    try {
        ...
        const keyPair = await curve.keyPair(privateKey);
    ...
    try {    
        ...
        const aliceShared = await curve.sharedSecret(bobKeyPair.pubKey, aliceKeyPair.privKey);
        const bobShared = await curve.sharedSecret(aliceKeyPair.pubKey, bobKeyPair.privKey);
    ...
    try {
        ...
        const aliceSharedCurve = await curve.sharedSecret(bobKeyPair.pubKey, aliceKeyPair.privKey);
    ...
    try {
        ...
        const signature = await curve.sign(keyPair.privKey, message);
        ...
        const isValid = await curve.verify(keyPair.pubKey, message, signature);
        assert(isValid === true, 'Signature verification should succeed');
        
        // Test invalid signature
        const invalidMessage = new Uint8Array([5, 4, 3, 2, 1]);
        const isInvalid = await curve.verify(keyPair.pubKey, invalidMessage, signature);
